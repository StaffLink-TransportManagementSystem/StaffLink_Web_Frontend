...
function submit() {
    ...
    if (checker === true) {
        ...
        formData.append("seatsCount", seatCount);
        formData.append("startingPoint", startingLocation);
        formData.append("endingPoint", endLocation);
        formData.append("trips", vehicleTrips);
        formData.append("frontImage", frontImage.files[0]);
        formData.append("sideImage", sideImage.files[0]);
        formData.append("backImage", backImage.files[0]);
        formData.append("insideImage", insideImage.files[0]);
        formData.append("certificate", certificate.files[0]);
        formData.append("insurance", insurance.files[0]);

        fetch('http://localhost:8080/try2_war_exploded/vehicleRegisterWithImages', {
            method: 'POST',
            body: formData
        })
            .then(response => response.json())
